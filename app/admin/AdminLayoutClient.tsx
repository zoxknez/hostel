...
export default function AdminLayoutClient({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-[#02040a] flex flex-col md:flex-row">
            ...
            {/* Main Content */}
            <main className={`flex-1 overflow-y-auto w-full ${!isLoginPage ? 'md:pl-64' : ''}`}>
                <div className={`p-4 md:p-12 max-w-7xl mx-auto ${isLoginPage ? 'flex items-center justify-center min-h-screen' : ''}`}>
                    {children}
                </div>
            </main>
        </div>
